...
class FlinkDataSourceInfo extends JDBCDataSourceInfo {

    private static final Log log = Log.getLog(FlinkDataSourceInfo.class);

    private String serverVersion;
    private String clientVersion;

    FlinkDataSourceInfo(DBRProgressMonitor monitor, DBPDataSource dataSource, JDBCDatabaseMetaData metaData) {
        super(metaData);

        try (JDBCSession session = DBUtils.openMetaSession(monitor, dataSource, "Read Flink version")) {
            serverVersion = JDBCUtils.queryString(session, "SELECT version()");
        } catch (Exception e) {
            log.debug("Error reading Flink version: " + e.getMessage());
        }
        try {
            ...
            log.debug("Error getting Flink client version");
        }
    }
